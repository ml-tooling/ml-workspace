define([
  ...
  function load_ipython_extension() {
    ...
    btGitButton =
      '<div id="start-git-btn" style="margin-right: 5px;">' +
      '<button id="gitTreeButton" class="btn btn-default btn-xs" style="padding: 5px 10px;">' +
      "<span>Git</span>" +
      "</button></div>";

    var vsCodeButton =
      '<button id="vsCodeTreeButton" title="Open VS Code" aria-label="Open VS Code" style="margin-left: 4px;" class="btn btn-default btn-xs"><i style="font-weight: bold;" class="fa-code fa"></i></button>';
    ...
    $("#vsCodeTreeButton").click(function () {
      ...
      window.open(
        basePath + "tools/vscode/?folder=" + encodeURIComponent(tree_dir),
        "_blank"
      );
    });

    var btGitButtonInTabView =
      '<button id="gitTreeButton" title="Open Git Helper" aria-label="Open Git Helper" style="margin-left: 4px;" class="btn btn-default btn-xs"><i class="fa-git fa"></i></button>';
    ...
    $(".dynamic-buttons:first").append(
      '<button id="#commit-push-button" title="Commit and push file" style="margin-right: 4px;" class="commit-push-button btn btn-default btn-xs">Commit & Push</button>'
    );
    $(".dynamic-buttons:first").append(
      '<button id="#share-data-button" title="Share data" style="margin-right: 4px;" class="share-data-button btn btn-default btn-xs"><i class="fa fa-share-alt"></i></button>'
    );
    $(".dynamic-buttons:first").append(
      '<button id="#vs-code-button" title="Open VS Code" style="margin-right: 4px;" class="vs-code-button btn btn-default btn-xs">VS Code</button>'
    );
    ...
    $(".vs-code-button").click(function () {
      window.open(
        basePath +
          "tools/vscode/?folder=/workspace/" +
          Jupyter.notebook_list.selected[0].path,
        "_blank"
      );
    });
